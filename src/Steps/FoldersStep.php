...
class FoldersStep extends BaseStep
{
    ...
    public function getFormData()
    {
        ...
        foreach ($folders as $path => $perm) {
            if ($this->checkFolderPermission($path, $perm)) {
                $formData['folders'][] = [
                    'label' => trans('install_wizard::steps.folders.view.granted', ['path' => $path, 'perm' => $perm]),
                    'check' => true,
                ];
            } else {
                $formData['folders'][] = [
                    'label' => trans('install_wizard::steps.folders.view.missing', [
                        'path' => $path,
                        'perm' => $perm,
    ...
    {
        return config('install_wizard.folder_permissions');
    }
