...
class EnvFileStep extends BaseStep
{
    ...
    function apply($formData)
    {
        ...
        if (false === file_put_contents($envFilePath, $formData['file_content'])) {
            $this->addError('env.errors.cannot_write_file', trans('install_wizard::steps.env.errors.cannot_write_file'));
    ...
    public function undo()
    {
        ...
        if (file_exists($envFile)){
            ...
            if (false===rename($envFile, $backupFile)){
                $this->errors->add('env.errors.cannot_backup_file', trans('install_wizard::steps.env.errors.cannot_backup_file'));
